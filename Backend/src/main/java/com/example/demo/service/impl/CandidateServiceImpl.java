...
public class CandidateServiceImpl implements CandidateService {
    ...
    public CandidateResponseDto createCandidate(CandidateRequestDto dto) {
        // If resume URL or text provided, try to enrich the DTO with parsed fields
        try {
            CandidateRequestDto enriched = enrichDtoFromResume(dto);
            Candidate candidate = mapToEntity(enriched);
            return convertToResponseDto(candidateRepository.save(candidate));
        } catch (Exception ex) {
            // On any parsing/enrichment failure, fall back to saving provided fields
